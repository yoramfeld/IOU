      ...
      sessions.map(s =>
        fetch(`/api/balances?groupId=${s.groupId}`)
          ...
          .then((data: { id: string; balance: number }[]) => {
            ...
            return { groupId: s.groupId, balance: member ? member.balance : 0 }
          })
          .catch(() => ({ groupId: s.groupId, balance: null as null }))
      )
    ).then(results => {
      ...
      <div className="w-full max-w-xs">
        ...
        <div className="space-y-2 mb-6">
          {sessions.map(s => {
            ...
            const isZero = balLoaded && bal !== null && Math.abs(bal) < 0.01
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <p className={`text-xs mt-0.5 ${
                        ...
                        {!balLoaded || bal === null ? '—' : `${bal >= 0 ? '+' : ''}${fmt(bal)}`}
                      </p>
                    </button>
                    {isZero && (
                      <button
